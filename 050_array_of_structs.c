...
int main() {
  ...
  printf("length_of_max_name = %d", length_of_max_name);

  for (int counter = 0; counter < (length_of_max_name - strlen("Name")); counter++) {
    printf(" ");
  }
  printf("Name");
  for (int counter = 0; counter < (length_of_max_name - strlen("ID number")); counter++) {
    printf(" ");
  }
  printf("ID number");
  for (int counter = 0; counter < (length_of_max_name - strlen("Aggregate Marks")); counter++) {
    printf(" ");
  }
  ...
  for (int count = 0; count < num_of_students; count++) {
    for (int counter = 0; counter < length_of_max_name - strlen(stud_arr[count].name); counter ++) {
      printf(" ");
    }
    printf("%s", stud_arr[count].name);
    for (int counter = 0; counter < length_of_max_name - log10(stud_arr[count].ID_number) + 1; counter ++) {
      printf(" ");
    }
    printf("%i", stud_arr[count].ID_number);
    for (int counter = 0; counter < length_of_max_name - log10(stud_arr[count].agg_marks) + 1; counter ++) {
      printf(" ");
    }
